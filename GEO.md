...
引擎會很有信心地引用一個過期的前提。2026-09-25 起前兩處已經不必人工同步（它們自己算）：

1. `public/llms.txt`——**程式產生**（`transform/emit-llms-full.mjs` 的 `navDoc()`），
   數字由 `data/page/about.json` 插值。要改的是措辭，不是數字；直接編輯產出的 txt 會被下次 build 蓋掉。
2. `src/pages/about.astro`——涵蓋範圍、覆蓋率、零售倍數、價格鏈三態全部讀 `about.json`，頁面裡沒有寫死的值。
3. `README.md` 的資料來源盤點——不寫現況數字，只留查法（見該檔開頭）。
4. 本文件開頭的「這一軸在防什麼」——只寫**口徑規則**（單位、基準、產地價不是零售價），不寫數字。

所以口徑改變時真正要動手的是：`transform/emit-page.mjs` 算 `about.json` 的那一段（新增一個要講的事實）、
以及上面 3、4 兩處的規則敘述。數字自己會跟上。
